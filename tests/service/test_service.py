...
def test_invoke_model_param(test_client, mock_agent) -> None:
    ...
    with patch(
        "service.service.settings.AVAILABLE_MODELS",
        {CUSTOM_MODEL},
    ):
        response = test_client.post(
            "/invoke",
            json={"message": QUESTION, "model": CUSTOM_MODEL},
        )
        assert response.status_code == 200
...
def test_history_returns_empty_list_for_new_thread(test_client, mock_agent) -> None:
    mock_agent.aget_state.return_value = StateSnapshot(
        values={},
        next=(),
        config={},
    ...
    assert response.status_code == 200
    assert ChatHistory.model_validate(response.json()).messages == []
...
@pytest.mark.asyncio
async def test_stream_ignores_empty_supervisor_updates(test_client, mock_agent) -> None:
    """Supervisor graphs may emit update events without a message payload."""
    final_answer = "The supervisor completed successfully."
    events = [
        ("updates", {"supervisor": {"messages": []}}),
        ("updates", {"chat_model": {"messages": [AIMessage(content=final_answer)]}}),
    ]
    ...
    with test_client.stream(
        "POST", "/stream", json={"message": "Test empty update", "stream_tokens": False}
    ) as response:
        messages = [
