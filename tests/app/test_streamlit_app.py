...
@pytest.mark.asyncio
async def test_app_streaming_renders_workflow_events_without_taskdata_error(
    mock_agent_client,
):
    at = AppTest.from_file("../../src/streamlit_app.py").run()
    ...
    messages = [
        ...
        ChatMessage(
            ...
            custom_data={
                ...
                "phase": "architect",
                "status": "completed",
                "detail": "grounded symbol found",
            },
        ),
        ChatMessage(type="ai", content="Architecture complete"),
    ]
    ...
    assert any(status.label == "Architect" for status in at.status)
    assert at.chat_message[-1].markdown[-1].value == "Architecture complete"
    assert not at.error
    assert not at.exception


@pytest.mark.asyncio
async def test_app_streaming_skips_human_echo_between_tool_call_and_result(
    mock_agent_client,
):
    at = AppTest.from_file("../../src/streamlit_app.py").run()
    prompt = "Run a tool"
    messages = [
        ...
        ChatMessage(type="human", content=prompt),
        "tool token that must not enter session history",
        ChatMessage(type="tool", content="42", tool_call_id="tool-call"),
    ...
    assert len([message for message in at.chat_message if message.avatar == "user"]) == 1
    assert at.chat_message[1].markdown[-1].value == "The answer is 42"
    assert all(
...
@pytest.mark.asyncio
async def test_app_streaming_sub_agent_stream_ends_early(
    mock_agent_client, multi_agent_messages
):
    """A truncated sub-agent stream should mark its status as failed without crashing the app."""
    at = AppTest.from_file("../../src/streamlit_app.py").run(timeout=10)
    messages = multi_agent_messages

    async def amessage_iter():
        yield messages["transfer_a"]
        yield messages["transfer_a_success"]
        yield "partial hardware output"

    mock_agent_client.astream = Mock(return_value=amessage_iter())

    at.toggle[0].set_value(True)
    at.chat_input[0].set_value("Test truncated sub-agent stream").run(timeout=10)

    status_agent = at.chat_message[1].status[0]
    assert status_agent.markdown[-1].value == "Sub-agent stream ended before control returned."
    assert all(
        isinstance(message, ChatMessage)
        for message in at.session_state.messages
    )
...
@pytest.mark.asyncio
async def test_app_streaming_skips_human_echo_before_sub_agent_ack(
    mock_agent_client,
    ...
    async def amessage_iter():
        for message in [
            messages["transfer_a"],
            ChatMessage(type="human", content=prompt),
            "handoff token that must not enter session history",
            messages["transfer_a_success"],
            messages["transfer_back_a"],
            messages["transfer_back_a_success"],
            messages["supervisor_final"],
    ...
    assert len([message for message in at.chat_message if message.avatar == "user"]) == 1
    assert at.chat_message[1].markdown[-1].value == (
...
@pytest.mark.asyncio
async def test_app_streaming_sub_agent_tokens(mock_agent_client, multi_agent_messages):
    """String tokens from a sub-agent should be rendered in its status container."""
    at = AppTest.from_file("../../src/streamlit_app.py").run(timeout=10)
    messages = multi_agent_messages
    sub_agent_final = ChatMessage(type="ai", content="Streaming response")
    ...
    at.toggle[0].set_value(True)
    at.chat_input[0].set_value("Test sub-agent token streaming").run(timeout=10)

    status_agent = at.chat_message[1].status[0]
    assert status_agent.markdown[0].value == "Streaming response"
    assert not at.exception
