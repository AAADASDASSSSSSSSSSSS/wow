...
class SchematicDoc:
    ...
    def pin_geometry(self, reference: str) -> list[dict[str, Any]] | None:
        """Resolved pins of a placed symbol, or None when unresolvable.

        Prefers the schematic's own embedded ``lib_symbols`` definition, which is
        what makes this work for projects that ship their own library rather than
        using the installed KiCad ones.
        """
        try:
            return self._sch.pin_locations(reference)
        except ValueError:
            return None
