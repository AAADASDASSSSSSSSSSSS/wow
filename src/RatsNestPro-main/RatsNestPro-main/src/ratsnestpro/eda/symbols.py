...
def symbol_roots() -> list[Path]:
    ...
    KiCad install locations are discovered when it is unset **and** when every
    path it names is absent, because a stale value is no better than no value.
    Only existing directories are returned.

    That second condition is the whole reason this docstring is long.
    :func:`ratsnestpro.config.symbol_dir` resolves a stale value exactly this
    way, and while the two disagreed the answer to "where are the symbols"
    depended on who asked: config fell back to the installed KiCad, this
    resolver returned an empty list. An empty root list is silent — every pin
    lookup returns ``None``, so pin geometry, alternate functions and pad counts
    all quietly stop being verified rather than reporting that they cannot be.
    One env var left pointing at a moved library was enough to trigger it.
    """
    roots: list[Path] = []
    ...
    if not any(r.exists() for r in roots):
        # Nothing configured, or nothing configured that still exists.
        ...
        pins.append(
            {
                ...
                # Alternate functions the library declares for this pin, e.g.
                # ``RCC_OSC_IN`` on an STM32's PD0. This is the only place a
                # peripheral channel is named: the default pin name is the port
                # bit (``PC14``), which says nothing about which oscillator it
                # belongs to. Empty for symbols that declare none.
                "alternates": tuple(
                    str(alt[1])
                    for alt in find_all(pin, "alternate")
                    if len(alt) > 1 and str(alt[1])
                ),
            }
        )
...
def _pins_for(nick: str, name: str, depth: int = 0) -> list[dict[str, Any]]:
    ...
    if base:
        return _pins_for(nick, base, depth + 1)
    return []
...
def symbol_pins(lib_id: str) -> list[dict[str, Any]] | None:
    ...
    de-duplicated by pin number, keeping the first occurrence.
    """
    if ":" not in lib_id:
        return None
    nick, name = lib_id.split(":", 1)
    pins = _pins_for(nick, name)
    if not pins:
        return None
...
def _properties_for(nick: str, name: str, depth: int = 0) -> dict[str, str]:
    ...
    base = _extends_base(symbol)
    if base:
        inherited = _properties_for(nick, base, depth + 1)
...
def symbol_properties(lib_id: str) -> dict[str, str]:
    ...
    if ":" not in lib_id:
        return {}
    nick, name = lib_id.split(":", 1)
    return _properties_for(nick, name)
