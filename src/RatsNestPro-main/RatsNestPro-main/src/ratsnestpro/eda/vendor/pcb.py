...
class PcbBoard:
    ...
    def list_nets(self) -> List[Dict[str, Any]]:
        """Every net on the board, in either of KiCad's two layouts.

        Boards at format ``20250907`` and earlier declare each net once at the
        top level as ``(net INDEX "NAME")``, and a pad references the index.
        From ``20251101`` onward that table is gone: a pad carries
        ``(net "NAME")`` directly and the board's net list is whatever its pads
        name. Both are current — KiCad 10 ships ``CM5_MINIMA_3`` in the old
        layout and ``pic_programmer`` in the new one.

        Reading only the top-level table reported **zero** nets for every board
        pcbnew 10 saves, including this pipeline's own routed output, which left
        every board-level check with an empty netlist and no way to tell that
        apart from a board that genuinely has none.

        ``index`` for a new-layout board is this enumeration's position, not a
        board-level identifier: the format has none. Do not write it back.
        """
        declared: List[Dict[str, Any]] = []
        for n in self._net_nodes():
            if len(n) >= 3:
                declared.append(
                    {"index": Atom(str(n[1])).as_int(), "name": str(n[2])}
                )
        if declared:
            return declared
        names: List[str] = []
        ...
        for fp in find_all(self.root, "footprint"):
            for pad in find_all(fp, "pad"):
                node = find_first(pad, "net")
                # Exactly one payload and no index: the new layout. A two-token
                # ``(net INDEX)`` also has one payload, but that shape only
                # appears on segments and vias, never on a pad.
    ...
    def _net_name_of_reference(self, node: Optional[list]) -> Optional[str]:
        ...
        The two board layouts reference a net differently: before format
        ``20251101`` a track carries the index from the top-level table
        (``(net 1)``), and from ``20251101`` it carries the name
        (``(net "GND")``). Both are one payload, so the shape alone does not say
        which, and reading an index as an integer on a new-layout board raises.
        Resolved to the name in both cases so callers need not know.
        """
        if node is None or len(node) < 2:
            return None
        raw = str(node[1])
    ...
    def list_tracks(self, net: Optional[Any] = None,
        ...
        """Copper segments, with each one's net reported by name.

        ``net`` filters by name or by old-layout index; both are resolved to a
        name before comparing, so the same call works on either board layout.
        """
        out = []
        want: Optional[str] = None
        if net is not None:
            want = self._net_name_of_reference([Atom("net"), str(net)])
        for seg in find_all(self.root, "segment"):
            ...
            seg_net = self._net_name_of_reference(find_first(seg, "net"))
            if layer is not None and seg_layer != layer:
                continue
            if want is not None and seg_net != want:
                continue
            out.append({
