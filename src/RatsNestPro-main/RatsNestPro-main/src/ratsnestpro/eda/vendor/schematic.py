...
class Schematic:
    ...
    def _embedded_symbol_pins(self, lib_id: str) -> Optional[List[Dict[str, Any]]]:
        """Pins from this schematic's own ``lib_symbols`` cache, or None.

        KiCAD 6+ embeds the full definition of every symbol it places, which is
        how KiCAD itself resolves geometry — the global libraries are only
        consulted when a symbol is first pulled in. Reading the cache first
        matters for any project that uses its own library: across the KiCAD demo
        corpus only 18% of placed symbols resolve against the installed
        libraries, while 98% of the embedded definitions carry pins. Ignoring the
        cache silently produced empty pin sets, and every check that needs pin
        geometry then passed by having nothing to look at.
        """
        from .symbol_lib import _pins_in

        libnode = find_first(self.root, "lib_symbols")
        if libnode is None:
            return None
        for child in libnode:
            if not isinstance(child, list) or tag_of(child) != "symbol":
                continue
            if len(child) > 1 and str(child[1]) == lib_id:
                pins = _pins_in(child)
                if not pins:
                    return None
                # De-duplicate by pin number, keeping the first occurrence, to
                # match symbol_lib.symbol_pins.
    ...
    def pin_locations(self, reference: str) -> Optional[List[Dict[str, Any]]]:
        ...
        if not lib_id:
            return None
        pins = self._embedded_symbol_pins(lib_id)
        if pins is None:
