...
def _resolve_in_roots(nick: str, roots: tuple) -> Optional[Path]:
    for root in roots:
        cand = Path(root) / f"{nick}.kicad_sym"
        if cand.exists():
            return cand
    return None
...
def resolve_symbol_library(nick: str) -> Optional[Path]:
    """Locate ``<nick>.kicad_sym``.

    Cached on the (nickname, root set) pair rather than the nickname alone, so
    pointing ``KICAD_SYMBOL_DIR`` somewhere else still yields a fresh answer.
    """
    return _resolve_in_roots(nick, tuple(str(r) for r in symbol_roots()))


@lru_cache(maxsize=128)
def _load_lib_node(path_str: str, stamp: tuple) -> Optional[list]:
    """Parse a ``.kicad_sym`` file, cached per (path, mtime, size).

    Without this, :func:`symbol_pins` re-read and re-parsed the whole library on
    every call. Building a connectivity graph asks for pin geometry once per
    component, so a 200-part board parsed a multi-megabyte library 200 times —
    which dominated every check that needed real pin coordinates. The sibling
    resolver in :mod:`ratsnestpro.eda.symbols` already caches the same way.

    ``stamp`` is not read; it is part of the cache key so an edited library is
    re-parsed instead of served stale.
    """
    del stamp
    try:
        node = loads(Path(path_str).read_text(encoding="utf-8"))
...
def _lib_node(path: Path) -> Optional[list]:
    try:
        st = path.stat()
    except OSError:
        return None
    return _load_lib_node(str(path), (st.st_mtime_ns, st.st_size))
