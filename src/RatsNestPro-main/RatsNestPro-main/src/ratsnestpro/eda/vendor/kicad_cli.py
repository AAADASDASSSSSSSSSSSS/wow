...
class KicadCli:
    ...
    def export_netlist(
        ...
        """Export the netlist KiCad itself would use.

        Resolves the whole sheet hierarchy from the root sheet in one call, so
        the caller must not iterate child sheets.
        """
        return self._run(
            ["sch", "export", "netlist", "--format", fmt, "--output", out_path, sch_path],
            produced=[out_path],
        )
