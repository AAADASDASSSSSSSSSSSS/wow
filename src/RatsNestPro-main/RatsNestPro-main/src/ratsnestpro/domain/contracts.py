...
class CircuitIR(ContractModel):
    ...
    # Pins deliberately left unconnected. Distinct from a pin that is merely
    # absent from every net: this records the *decision*, so a dangling-pin
    # check can tell "nobody thought about it" from "we decided it stays open".
    # A family that omits a GPIO from every net and does not list it here is
    # making no claim, and the checks are right to say so.
    no_connect_pins: list[PinRef] = Field(default_factory=list, max_length=2_000)
