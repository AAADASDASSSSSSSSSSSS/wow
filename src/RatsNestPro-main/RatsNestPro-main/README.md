...
.\.venv\Scripts\python.exe -m pytest -q -m "not real_kicad and not slow_corpus"
...
# 最大的几块 demo 板(数十 MB,单独跑)
.\.venv\Scripts\python.exe -m pytest -q -m slow_corpus

# 外部 fixture 语料(160 MB 第三方数据,放在仓库外);不设即整组 skip
$env:RATSNESTPRO_FIXTURE_HOME='C:\Users\<you>\kicad-fixtures'
```

三个 opt-in 标记各自的含义:`real_kicad` 需要装好的 KiCad 10 CLI,`real_ericai` 需要可达的
EricAI 网关,`slow_corpus` 是数十 MB 的 demo 板。**默认门禁必须同时排除 `real_kicad` 与
`slow_corpus`** —— 只排除前者会让 `test_oversized_boards_parse` 照样解析
`jetson-agx-thor-baseboard`(86 MB)和 `vme-wren`(70 MB),白花约 200 s。

当前实测(2026-08-03):**792 passed / 1 skipped / 10 deselected,约 174 s**。
`ruff check .` 有 **5 个既有错误**(`eda/factsheet.py` 的 UP046/E501,`tests/test_fact_sheets_data.py`
的 E501),`mypy src` 有 **17 个既有错误 / 56 个源文件** —— 两者都来自未提交的 fact-sheet 工作,
不是新增。**门禁的判据是这两个数字不增加**,不是零。
