    ...
    return [
        {
            ...
            "href": (f"https://www.st.com/resource/en/datasheet/{document_id.lower()}.pdf"),
            "body": (
...
def _validate_public_https_url(
    ...
    *,
    ...
    if not global_addresses or (not allow_mixed_dns and len(global_addresses) != len(addresses)):
...
def _download_proxy_text(url: str) -> tuple[str, str]:
    ...
    with (
        ...
        client.stream("GET", proxy_url) as response,
    ):
        response.raise_for_status()
        content_length = int(response.headers.get("content-length", "0") or 0)
...
def _proxy_sections(text: str) -> tuple[int | None, list[tuple[int | None, str]]]:
    ...
    document_pages = int(page_count_match.group(1)) if page_count_match else None
    ...
    markers = [
        ...
        if (
            ...
            and (document_pages is None or int(match.group(2)) == document_pages)
        )
    ]
    if not markers:
        chunk_size = 12_000
        return document_pages, [
            (None, body[start : start + chunk_size]) for start in range(0, len(body), chunk_size)
        ]
    ...
    return document_pages, [
        (page, "\n".join(segments)) for page, segments in sorted(page_text.items())
    ]
...
def _read_datasheet_via_text_proxy(
    ...
    ranked_pages = [(_page_score(section, terms), page, section) for page, section in sections]
...
def fetch_datasheet(url: str, query: str, max_pages: int = 5) -> str:
    ...
    try:
        ...
        try:
            ...
            result["direct_fetch_error"] = f"{type(direct_exc).__name__}: {direct_exc}"
            ...
            result = {
                ...
                "direct_fetch_error": (f"{type(direct_exc).__name__}: {direct_exc}"),
