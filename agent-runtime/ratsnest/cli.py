...
def cmd_design(args) -> int:
    ...
    if args.backend in ("mcp", "crew"):
        ...
        if args.backend == "crew":
            from ratsnest.crews import CreatorCrew
            CreatorCrew(config, gen_recorder).generate(spec, out_dir, strategy)
            say(f"generated by the creator crew (in-process KiCad skills): {out_dir}")
        else:
            from ratsnest.mcp_exec import KiCadMcpBackend
            KiCadMcpBackend(config, gen_recorder).generate(spec, out_dir, strategy)
            say(f"generated via KiCAD MCP sub-agents (stdio transport): {out_dir}")
    else:
