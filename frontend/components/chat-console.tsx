...
import {
  RatsNestDecision,
  canonicalDecisionReply,
  parseRatsNestDecisionBlock,
} from "@/lib/ratsnest-decisions";
...
const CONTINUATION_RUN_KEY = "ratsnest.continuation-run";
...
export function ChatConsole({ team, onEditTeam }: { team: TeamConfig; onEditTeam: () => void }) {
  ...
  const legacyDecisionPending = useRef(false);
  const [decisionSubmission, setDecisionSubmission] = useState<{ clientId: string; status: "submitting" | "submitted" | "error"; error?: string } | null>(null);
  ...
  function rememberContinuation(runId: string | null, selectedThread = threadId) {
    setContinuationRunId(runId);
    const key = `${CONTINUATION_RUN_KEY}:${selectedThread}`;
    if (runId) localStorage.setItem(key, runId); else localStorage.removeItem(key);
  }
  ...
  useEffect(() => {
    ...
    setContinuationRunId(safeUuid(localStorage.getItem(`${CONTINUATION_RUN_KEY}:${requestedThread}`)));
    void (async () => {
      try {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setStatus(legacyDecisionPending.current ? "等待你的工程决策" : selectedProfile ? "就绪" : "请选择能力范围");
      } catch {
        if (!controller.signal.aborted) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const displayed = parsed.map((item) => displayMessage(item));
        setMessages(displayed);
        const lastHumanIndex = displayed.findLastIndex((item) => item.type === "human" && !item.pending);
        legacyDecisionPending.current = displayed.some(
          (item, index) => item.type === "ai" && index > lastHumanIndex && parseRatsNestDecisionBlock(item.content) !== null,
        );
        if (legacyDecisionPending.current) {
        ...
        try {
          const latestResponse = await fetch(
            `/api/projects/${encodeURIComponent(workspace.project.projectId)}/threads/${encodeURIComponent(threadId)}/latest-run?organization_id=${encodeURIComponent(workspace.organization.tenantId)}`,
            { cache: "no-store", signal: controller.signal },
          );
          if (latestResponse.ok) {
            const latest = parseRunSummary(await latestResponse.json());
            if (latest && latest.projectId === workspace.project.projectId && latest.threadId === threadId) {
              setLatestRun(latest);
              rememberContinuation(latest.runId, threadId);
            }
          }
        } catch {
          // localStorage remains the bounded fallback if lineage lookup is unavailable.
        }
      } catch (error) {
        if (!controller.signal.aborted) {
          setMessages([]);
  ...
  async function loadDelivery(runId: string): Promise<void> {
    ...
    try {
      const runResponse = await fetch(`/api/runs/${encodeURIComponent(runId)}?${query}`, { cache: "no-store" });
      if (!runResponse.ok) throw new Error("run metadata unavailable");
      const run = parseRunSummary(await runResponse.json());
      if (!run || run.runId !== runId) throw new Error("invalid run metadata");
      setLatestRun(run);
      rememberContinuation(runId);
      const artifactResponse = await fetch(`/api/runs/${encodeURIComponent(runId)}/artifacts?${query}`, { cache: "no-store" });
      if (!artifactResponse.ok) throw new Error("artifact metadata unavailable");
      const manifest = parseArtifactList(await artifactResponse.json());
      if (!manifest || manifest.runId !== runId) throw new Error("invalid artifact metadata");
  ...
  function handleRunEvent(event: RunEvent): HumanInputRequest | null {
    ...
    if (event.type === "message") {
      ...
      if (structured.type === "ai") {
        ...
        if (parseRatsNestDecisionBlock(completed.content)) {
          legacyDecisionPending.current = true;
          setRunState("waiting_for_input");
          setStatus("等待你的工程决策");
        }
    ...
    if (isTerminalRunEvent(event.type)) {
      rememberContinuation(event.runId);
  ...
  async function submit(event?: FormEvent, decision?: { message: string; baseRunId: string; clientId: string }) {
    event?.preventDefault();
    const message = (decision?.message ?? draft).trim();
    ...
    const activeRunProfileReference = latestRun?.capabilityProfile
      ...
      : latestRun
        ? null
        : undefined;
    const newProject = decision ? false : requiresNewRun(
      message,
      effectiveProfileReference,
      activeRunProfileReference,
    );
    const submissionThreadId = newProject ? newId() : threadId;
    const baseRunId = decision?.baseRunId ?? (newProject ? null : continuationRunId);
    if (newProject) {
      ...
      setLatestRun(null);
      rememberContinuation(null, submissionThreadId);
    }
    ...
    const submittedMessage = displayMessage(makeMessage("human", message), Boolean(decision));
    legacyDecisionPending.current = false;
    setMessages((current) => [
      ...(newProject ? [] : current),
      submittedMessage,
    ]);
    if (decision) setDecisionSubmission({ clientId: decision.clientId, status: "submitting" });
    setInteraction(null);
    ...
    try {
      for (let attempt = 0; attempt <= MAX_RECONNECTS && terminal === null; attempt += 1) {
        try {
          ...
          runId = responseRunId;
          accepted = true;
          rememberContinuation(runId, submissionThreadId);
          if (decision) {
            setMessages((items) => items.map((item) => item.clientId === submittedMessage.clientId ? { ...item, pending: false } : item));
            setDecisionSubmission({ clientId: decision.clientId, status: "submitted" });
          }
      ...
      if (decision && !accepted) {
        legacyDecisionPending.current = true;
        setMessages((items) => items.filter((item) => item.clientId !== submittedMessage.clientId));
        setDecisionSubmission({ clientId: decision.clientId, status: "error", error: error instanceof Error ? error.message : "提交失败" });
      }
      if (controller.signal.aborted) {
  ...
  async function submitDecision(clientId: string, decisions: RatsNestDecision[], selections: Record<string, string>, freeText: Record<string, string>): Promise<void> {
    if (busy || interaction || !continuationRunId) return;
    const answer = canonicalDecisionReply(decisions, selections, freeText);
    if (!answer) return;
    await submit(undefined, { message: answer, baseRunId: continuationRunId, clientId });
  }

  async function respondToInteraction(answer: string): Promise<void> {
    ...
    try {
      ...
      setMessages((items) => [...items, displayMessage(makeMessage("human", answer))]);
  ...
  function createThread() {
    ...
    legacyDecisionPending.current = false;
    rememberContinuation(null, nextThread);
  }
  ...
  const lastHumanIndex = allVisible.findLastIndex((message) => message.type === "human" && !message.pending);
  const actionableDecisionId = [...allVisible]
    .map((message, index) => ({ message, index }))
    .filter(({ message, index }) => message.type === "ai" && index > lastHumanIndex && parseRatsNestDecisionBlock(message.content))
    .at(-1)?.message.clientId ?? null;
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    if (event.target.value) {
                      ...
                      setStatus(legacyDecisionPending.current ? "等待你的工程决策" : "就绪");
                    } else {
            ...
            {authenticationRequired ? (
              ...
              visibleMessages.map((message) => (
                <MessageCard
                  ...
                  message={message}
                  decisionEnabled={message.clientId === actionableDecisionId && !busy && !interaction && Boolean(continuationRunId)}
                  decisionStatus={decisionSubmission?.clientId === message.clientId ? decisionSubmission : null}
...
function HumanInputCard({
  ...
  return (
    <article className="human-input-card" aria-label="智能体需要你的补充信息">
      ...
      {interaction.request.options.length > 0 && (
      ...
      {interaction.request.allowFreeText && (
        <textarea
          value={answer}
      ...
      <footer>
        ...
        {interaction.status !== "submitted" && (
          <button type="button" disabled={!canSubmit} onClick={() => void onRespond(answer.trim())}>
...
function MessageCard({
  ...
  decisionStatus: { clientId: string; status: "submitting" | "submitted" | "error"; error?: string } | null;
  onDecision: (clientId: string, decisions: RatsNestDecision[], selections: Record<string, string>, freeText: Record<string, string>) => Promise<void>;
}) {
  ...
  const decisionBlock = message.type === "ai" ? parseRatsNestDecisionBlock(message.content) : null;
  const shownContent = decisionBlock?.prose ?? message.content;
  return (
      ...
      <div className="message-body">
        ...
        {decisionBlock && (
          <DecisionCard
            ...
            decisions={decisionBlock.decisions}
...
function DecisionCard({ clientId, decisions, enabled, status, onSubmit }: {
  clientId: string;
  decisions: RatsNestDecision[];
  enabled: boolean;
  status: { status: "submitting" | "submitted" | "error"; error?: string } | null;
  onSubmit: (clientId: string, decisions: RatsNestDecision[], selections: Record<string, string>, freeText: Record<string, string>) => Promise<void>;
}) {
  const [selections, setSelections] = useState<Record<string, string>>({});
  const [freeText, setFreeText] = useState<Record<string, string>>({});
  const answer = canonicalDecisionReply(decisions, selections, freeText);
  const locked = !enabled || status?.status === "submitting" || status?.status === "submitted";
  return (
    <section className="decision-card" aria-label="RatsNest 需要你的工程决策">
      {decisions.map((decision, index) => {
        const selected = decision.options.find((option) => option.key === selections[decision.slot]);
        return (
          <fieldset key={decision.slot} disabled={locked}>
            <legend>{index + 1}. {decision.question}</legend>
            {decision.options.map((option) => (
              <label key={option.key}>
                <input type="radio" name={`${clientId}-${decision.slot}`} checked={selected?.key === option.key} onChange={() => setSelections((value) => ({ ...value, [decision.slot]: option.key }))} />
                <span><strong>{option.label}</strong>{option.key === decision.recommendedKey && <em>推荐</em>}{(option.basis || decision.citation) && <small>依据：{option.basis || decision.citation}</small>}</span>
              </label>
            ))}
            {selected?.freeText && <textarea rows={2} maxLength={10_000} value={freeText[decision.slot] ?? ""} onChange={(event) => setFreeText((value) => ({ ...value, [decision.slot]: event.target.value }))} placeholder="填写这一项的具体值或约束" />}
          </fieldset>
        );
      })}
      <footer>
        <span>{status?.status === "submitted" ? "已提交，将在同一会话继续" : status?.status === "error" ? status.error : enabled ? "可分多轮回答；未选择的项目会继续询问" : "历史决策或当前任务不可提交"}</span>
        {status?.status !== "submitted" && <button type="button" disabled={locked || !answer} onClick={() => answer && void onSubmit(clientId, decisions, selections, freeText)}>{status?.status === "submitting" ? "提交中" : status?.status === "error" ? "重试" : "确认并继续"}</button>}
      </footer>
    </section>
  );
}
