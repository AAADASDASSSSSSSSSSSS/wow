...
these transfers to the current Streamlit sub-agent status UI.
...
## Risk arbitration before a build starts

A `build` request is arbitrated against the device fact sheets before any design
work runs. `initialize` extracts the numeric values the requirement asks for
(`ratsnestpro.eda.factclaim`) and compares each against the asserted datasheet
slot it belongs to. When a value breaks a cited limit and has not been
acknowledged, the graph routes to `clarify_risk`, which states the value, the
limit, the page it comes from, and the exact `ACK-RISK:` token that accepts it —
then ends the turn. Nothing is generated in the meantime, so changing the value
costs nothing to unwind.

The user's next turn answers it in their own words. A model proposes which tokens
that reply accepts; deterministic code then discards any token that was not among
the ones actually offered, so an over-generous or hallucinated answer cannot waive
a limit the user never saw. An unreadable reply accepts nothing and the question
is asked again.

Once accepted, the token is appended to the requirement text, which is how the
decision reaches the pipeline: `RequirementsStep` parses the same token and
downgrades the same check from ERROR to WARNING. The check is downgraded, never
deleted, and the accepted risk is listed in the final report with the citation it
overrode — an accepted risk must stay distinguishable from a risk nobody found.

Only `build` is gated. A review, a parts lookup, or a research request cannot
...
- the existing Streamlit Agent selector discovers the system from `/info`.
...
Open the existing Streamlit app and select `ratsnestpro-multi-agent`.
Example requests:
